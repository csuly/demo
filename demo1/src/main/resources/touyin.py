...
def getDate(target_scene):
    ...
    result.columns = ['id', 'source', 'batch', 'time_min', 'time_max', 'duration', 'min_lon', 'max_lon', 'min_lat',
                      'max_lat', 'avg_vel', 'avg_accel', 'avg_cou', 'avg_anguvel', 'points', 'sparsity']
    # 打印查询结果
    # print(result)
    mycursor.close()
    mydb.close()
    ...
    try:
        for index, row in info.iterrows():
            update = "INSERT INTO `" + str(target_scene) + "-features-projection`" + \
                     ...
                     str(row['source']) + "," + str(row['batch']) + "," + str(row['time_min']) + "," + str(
                row['time_max']) + "," + str(row['duration']) + "," + \
                     str(row['min_lon']) + "," + str(row['max_lon']) + "," + str(row['min_lat']) + "," + str(
                row['max_lat']) + "," + str(row['avg_vel']) + "," + \
                     str(row['avg_accel']) + "," + str(row['avg_cou']) + "," + str(row['avg_anguvel']) + "," + str(
                row['points']) + "," + \
                     str(row['sparsity']) + "," + str(row['projection_x_ISOMAP']) + "," + str(
                row['projection_y_ISOMAP']) + "," + \
